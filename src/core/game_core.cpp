...
json make_error_response(const std::string& request_id, const std::string& code,
...
json make_success_response(const std::string& request_id, int contract_version, const json& state,
                           const json& events)
{
    json response;
    response["ok"] = true;
...
json handle_attack(const json& input)
{
    ...
    int contract_version = input.value("contract_version", -1);
    ...
    if (actor == nullptr)
    {
        return make_error_response(request_id, "ENTITY_NOT_FOUND", "Actor entity not found",
                                   {{"actor_id", source_id}});
    }

    if (target == nullptr)
    {
        return make_error_response(request_id, "ENTITY_NOT_FOUND", "Target entity not found",
                                   {{"target_id", target_id}});
    }
    ...
    return make_success_response(request_id, contract_version, state, events);
}
json handle_request(const json& input)
{
    ...
    {
        return make_error_response(request_id, "INVALID_ACTION", "Unsupported action type",
    ...
    {
        return make_error_response(request_id, "UNSUPPORTED_VERSION",
...
std::string handle_json(const std::string& input_json)
{
    ...
    {
        return make_error_response("", "INVALID_JSON", "Invalid JSON input", json::object()).dump();
    }
    catch (const json::exception&)
    {
        return make_error_response("", "INVALID_SCHEMA", "Request JSON does not match API contract",
                                   json::object())
            .dump();
    }
    catch (const std::exception&)
    {
        return make_error_response("", "INTERNAL_ERROR", "Internal C++ core error", json::object())
            .dump();
    }
}
